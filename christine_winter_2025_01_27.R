...
obs_yield <- read_excel("data/measured_crop/obs_yield.xlsx", sheet = "obs_yield_main")
...
obs_yield <- read_excel("C:/Users/aolivo/OneDrive - University of Guelph/0_all_files_postdoc/1_projects/1_modeling_div_conv/3_undergrad_christine/Christine Cudmore/1_winter_2025/obs_yield.xlsx", sheet = "obs_yield_dndc")
obs_yield$type <- "obs"
obs_yield$grain_t_ha = as.numeric(obs_yield$grain_t_ha)
obs_yield$crop_yield_obs_kg_ha <- obs_yield$grain_t_ha*1000
obs_yield_p1 <- filter(obs_yield, plot == "1")
...
combined_data <- merge(obs_yield_p3, mod_yield_p3, by = c("year", "crop_type"))
...
# graphs 
obs_n2o_p1$doy <- obs_n2o_p1$DOY

ggplot(data = mod_n2o_p1, aes(x = as.numeric(doy), y = n2o_flux_g_n_ha_d)) +
  geom_line(size = 1, color = "darkgray") +
  geom_line(data = obs_n2o_p1, aes(x = as.numeric(doy), y = N2O_.g_N.ha.d.), color = "#0072B2", size = 1, alpha = 0.6) +
